...
- 2026-08-21: Config de auth lista para el piloto salvo el SMTP: Site URL corregido (era `http://localhost:3000` — cada link de confirmación habría apuntado al localhost del vecino), plantilla de confirmación en español y apuntando a `{{ .SiteURL }}/auth/confirm?token_hash=...` (la de fábrica usaba `{{ .ConfirmationURL }}`, que no pasa por el route de la app y devuelve los tokens en el fragmento, ilegible para SSR). Pendiente: SMTP propio, sin el cual el registro está topado a 2/hora
- 2026-08-20: Rate limiting con dos caracteres distintos a propósito: el techo de avistamientos por vecino (20/h) **rechaza** el envío y corre antes de subir la foto; los techos de análisis de IA (8/h por vecino, 25/h por reporte) solo **saltan el score** — el aviso se guarda, la fila de match se crea y el dueño revisa a mano. Degradar el lujo antes que negar la función. Ante un error de la consulta fallan en direcciones opuestas: el primero deja pasar (no silenciar a quien ayuda), el segundo bloquea (no regalar crédito de API)
